...
- `policy/` … 利用規程（同意対象）。
- `docs/` … クイックスタート（`quickstart.md`）・MCP導入（`mcp-setup.md`）・効力テスト（`testing.md`）。
...
managed設定の `strictKnownMarketplaces` で**この2つ以外を禁止**（`anthropics/skills` は `extraKnownMarketplaces` で自動登録）。
